...
**Never send your final answer via `send_message`.** It is only for brief progress acks that are *different* from your result (e.g. "On it…"). Your final answer belongs in exactly one final `<message>` block — pushing the same content through both `send_message` and a `<message>` block delivers it twice and the user sees a duplicate reply.
